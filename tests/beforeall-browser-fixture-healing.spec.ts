...
// Temporary smoke test for tamash-playwright's bindBrowser() fix (0.13.0-beta.1, still current in
// 0.13.0-beta.3) -- verifies, through the REAL published package (installed from npm, not a
// same-repo source import), that a context/page built directly off the `browser` fixture in
// test.beforeAll (bypassing the context/page fixtures entirely -- the exact pattern a real support
// report used) still heals. Carried over from the earlier 0.13.0-beta.1 test branch onto this
// 0.13.0-beta.3 one, which started fresh from master and otherwise wouldn't re-exercise this fix.
